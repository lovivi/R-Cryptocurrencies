...
#Obtain list of currencies used on Kraken
kraken_assets_list<- function(){
...
kraken_live <- function(basecurrency = "XBT", pricecurrency = "USD", field = "all"){
  ...
  kraken_assets <- kraken_assets_list()

    if ((basecurrency %in% kraken_assets) == FALSE){
    print("Please input one of the currency among these ones:")
    print(kraken_assets)
    exit()
    }
  ...
  if (field == "all"){  #subset if field is not equal to "all"
    kraken_data_subset <- kraken_data
    colnames(kraken_data_subset) <- paste0(basecurrency, pricecurrency)
  }else{
    kraken_data_subset <- kraken_data[field,]
  }
  
  print(kraken_data_subset)
}


#Obtain OHLC for a pair of currencies
kraken_OHLC <- function(basecurrency = "XBT", pricecurrency = "USD", interval = "1", start_time){
...
kraken_order_book <- function(basecurrency = "XBT", pricecurrency = "USD", count){
